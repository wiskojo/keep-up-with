...
def initial_turn_message(config: KeepUpWithConfig) -> str:
    return dedent(
        ...
        1. Situate yourself. Understand who you are, what your purpose is, what kind of agent you are, and how you should relate to the user and the world. Read your durable context in `{config.paths.workspace}`.
        2. Understand what you have access to. Learn how `cli` works. Run `cli --help`, make sure you can use it, and learn the events, inbox, tools, subscriptions, messaging, thread, and space commands.
        3. Understand what the user wants to keep up with. They have already gone through setup, so inspect the enabled subscriptions and message space before asking broad setup questions.
        4. Meet the user. Introduce yourself, get a feel for what they care about, and ask if anything important is unclear.
        """
    ).strip()
