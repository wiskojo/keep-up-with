...
- **Let the evidence pick:** After highlights, ask what the story actually supports. One useful fact → quick update. One coherent point that needs explanation, a visual, or a quote → update post. Multiple useful perspectives, artifacts, or deltas the user may return to → deep-dive thread.
- **An existing thread wins:** If the story already has a thread, append the delta there instead of choosing a new tier.
- **When in doubt, go lighter:** A tight update post the user reads beats a thread they skim. Record the tier and the reason in `notes.md`.
